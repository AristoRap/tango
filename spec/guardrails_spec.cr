...
describe "NIR/LIR node inventory" do
  it "pins the exact set of NIR node classes" do
    ...
    exprs.should eq %w(ArrayBuild ArrayGet ArrayNew ArrayPush ArraySet Assign BlockLiteral BoolLiteral Call Cast ChannelNew ChannelOp ClassRef CollectionEach CollectionFilter CollectionFold CollectionMap EnumMember ExceptionHandler ExceptionNew FloatLiteral HashFetch HashGet HashHasKey HashKeyAt HashNew HashSet If IndexedRead IndexedWrite InstanceVar IntLiteral Interpolation InvokeBlock Local MutexNew New NilLiteral Not Raise Select Size Spawn StringCharAt StringEachChar StringLiteral StringSplit StringToFloat StringToInteger TypeTest UnsupportedExpr ValueSequence).sort
    stmts.should eq %w(Block BlockArg BlockParam Break Class Def Enum FieldInitializer Next Param Return While).sort
  end
